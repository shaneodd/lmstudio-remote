...
(() => {
    ...
    */
    ...
    // ---------- Rendering -------------------------------------------------
// counter for unique message IDs (already declared above)
...
// set element id for scrolling target
        div.id = msgId;
        // main text (prompt or assistant reply)
        const p = document.createElement('p');
        p.textContent = msgObj.content;
        div.appendChild(p);
        // add scroll‑to‑top button for assistant messages only
        if (msgObj.role === 'assistant') {
            const topBtn = document.createElement('button');
            topBtn.textContent = "🔝";
            topBtn.style.marginLeft='0.5rem';
            topBtn.onclick = () => { document.getElementById(msgId).scrollIntoView({behavior:'smooth', block:'start'}); };
    ...
    */
    ...
    // Scroll to the first message (top of conversation)
    scrollTopBtn.onclick = () => {
        const firstMsg = chatContainer.querySelector('.message');
        if (firstMsg) {
    ...
    if (navActions) {
        ...
        navActions.appendChild(scrollTopBtn);
    }
